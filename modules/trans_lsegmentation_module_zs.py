...
class TransLSegmentationModuleZS(pl.LightningModule):
    ...
    def criterion(self, logit_mask, gt_mask):
        # logit_mask.shape = [2, 512, 480, 480]
        bsz = logit_mask.size(0)
        logit_mask = logit_mask.contiguous().view(bsz, 2, -1) # logit_mask.shape = [2, 2, 589822400]
        gt_mask = gt_mask.view(bsz, -1).long()   # gt_mask.shape = [2, 230400] = (2, 480x480)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            ##DEBUG EDIT BATCH_SIZE=1
            "--batch_size", type=int, default=1, help="size of the batches"
        )
        parser.add_argument(
            "--base_lr", type=float, default=2e-4, help="learning rate"
        )
        parser.add_argument("--momentum", type=float, default=0.9, help="SGD momentum")
        parser.add_argument(
            "--weight_decay", type=float, default=1e-5, help="weight_decay"
        )
        parser.add_argument(
