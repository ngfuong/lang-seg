...
class Options:
    def __init__(self):
        ...
        parser.add_argument(
            "--logpath", type=str, default="debug/test_lseg_zs", help="path to log file"
        )
...
def test(args):
    ...
    # DEBUG
    # change split to train to get loss
    dataloader = FSSDataset.build_dataloader(args.benchmark, args.bsz, args.nworker, args.fold, 'trn', args.nshot)
    ...
    with torch.no_grad():
        for idx, batch in enumerate(dataloader):
            ...
            ## ADD DEBUG
            # loss = model(pred_mask, target)
            # from fewshot/train and lsegmentation_module_zs def criterion()
            # logit_mask = pred_mask.argmax(dim=1)
            # bsz = logit_mask.size(0)
            # logit_mask = logit_mask.view(bsz, 2, -1)
            gt_mask = target.long()
            # gt_mask = target.view(1, 2, -1).long()

            # PRINT SHAPE DEBUG
            # print("Logit_mask size:", logit_mask.size(), "\tdtype: ", logit_mask.type)
            # print("Gt mask size:", gt_mask.size(), "\tdtype: ", gt_mask.type)
            # print("tmp_loss size: ", loss.size())
            # END
            tmp_loss = criterion(pred_mask, gt_mask)
            ...
            # ADD LOSS TO DEBUG
            average_meter.update(area_inter, area_union, class_info, loss=tmp_loss.detach().clone())
            average_meter.write_process(idx, len(dataloader), epoch=-2, write_batch_idx=1)
    ...
    Logger.info('Fold %d, %d-shot ==> Avg Loss: %5.2f \t mIoU: %5.2f \t FB-IoU: %5.2f' % (args.fold, args.nshot, np.mean(loss), test_miou.item(), test_fb_iou.item()))
